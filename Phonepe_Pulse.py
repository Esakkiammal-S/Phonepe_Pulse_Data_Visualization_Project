...
myconnection = pymysql.connect(host='localhost', user='wonder', passwd='Esakki2024', database="phonepe_project", port=3306)
...
Aggr_Trans1 = pd.DataFrame(table1, columns=(
...
Map_Trans1 = pd.DataFrame(table3, columns=(
...
Top_Trans1 = pd.DataFrame(table5,
                          columns=("State", "Year", "Quater", "Pincodes", "Transaction_count", "Transaction_amount"))
...
def Transaction_Trans_User(df, year):
    ...
    with col1:
        fig_amount = px.bar(ta2, x="State", y="Transaction_amount", title=f"{year} Transaction_Amount",
                            color_discrete_sequence=px.colors.sequential.BuGn_r)
        st.plotly_chart(fig_amount)

    with col2:
        fig_count = px.bar(ta2, x="State", y="Transaction_count", title=f"{year} Transaction_Count",
...
def Transaction_Trans_Aggr(df, Year, Quater):
    ...
    fig_amount = px.bar(tab2, x="State", y="Transaction_amount", title=f"{Year} AND {Quater}Transaction_Amount",
                        color_discrete_sequence=px.colors.sequential.BuGn_r)
    st.plotly_chart(fig_amount)

    fig_count = px.bar(tab2, x="State", y="Transaction_count", title=f"{Year} AND {Quater}Transaction_Count",
                       color_discrete_sequence=px.colors.sequential.Bluered_r)
    st.plotly_chart(fig_count)
...
def Transaction_User_Aggr(df, Year, Quater):
    ...
    fig_count = px.pie(df, names="Brand", title=" Brand Names & count", values="Transaction_count", width=600,
                       color_discrete_sequence=px.colors.sequential.RdBu)
    st.plotly_chart(fig_count)

    fig_per = px.pie(df, names="Brand", title=" Brand Names & Percentage", values="Percentage", width=600,
...
def Transaction_User_Map(df, Year, Quater):
    ...
    fig_regist_user = px.pie(df, names="State", title=" State Wise Registered Users", values="Registered_Users",
                             width=800, height=800, color_discrete_sequence=px.colors.sequential.RdBu)
    st.plotly_chart(fig_regist_user)

    fig_App_opens = px.pie(df, names="State", title=" State Wise App_Opens", values="App_opens", width=800, height=800,
...
def states_names(df, State):
    ...
    with col1:
        fig_pie_1 = px.pie(data_frame=df34, names="Transaction_type", values="Transaction_amount", width=600,
    ...
    with col2:
        fig_pie_2 = px.pie(data_frame=df34, names="Transaction_type", values="Transaction_count", width=600,
...
def Top_10_Brands_users(Table):
    ...
    fig_count = px.pie(df_12, names="Brand", title=" Top 10 Brands", values="Transaction count", width=600,
                       color_discrete_sequence=px.colors.sequential.RdBu)
    st.plotly_chart(fig_count)
...
def Top_10_State_pincode(Table):
    ...
    fig_pin = px.pie(df_12, names="Pincodes", title=" Top 10 Pincodes", values="Transaction amount", width=600,
...
def Top_10_state_trans(Table):
    ...
    fig_state = px.pie(df_12, names="State", title=" Top 10 State", values="Transaction amount", width=600,
...
def Top_10_Payment_Type(Table):
    ...
    fig_pay_type = px.pie(df123, names="Transaction_Type", title=" Top Transaction type", values="Transaction_Amount",
                          width=600, color_discrete_sequence=px.colors.sequential.Agsunset_r)
...
def Top_10_Trans_counts(Table):
    ...
    with col1:
        fig_amount = px.histogram(df_1, x="State", y="Transaction_amount", title=" Transaction_Amount", width=500,
    ...
    with col2:
        fig_count = px.histogram(df_11, x="State", y="Transaction_count", title=" Transaction_count", width=500,
...
def State_36_Trans(Table):
    ...
    fig_amount = px.histogram(df_1, x="State", y="Transaction_amount", title=" Transaction_Amount",
...
def State_36_Counts(Table):
    ...
    fig_count = px.bar(df_11, x="State", y="Transaction_count", title=" Transaction_count",
...
def Top_registered_users(Table, state):
    ...
    fig_reg = px.bar(df124, x="District Name", y="registered Users", title="Top 10 Registered Users",
...
def Top_10_App_opnes(Table, state):
    ...
    fig_app = px.bar(df124, x="District Name", y="App Opens", title="Top 10 App Opens",
